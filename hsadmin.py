...
def hs_list(args):
    ...
    is_ok = lambda x: True
    if args.localport:
        is_ok = lambda x: x.address.split(':')[1] == args.localport
    for hs in filter(is_ok, get_hidden_services()):
        print args.fmt % hs
...
def hs_add(args):
    ...
    hsdir = args.hsdir if args.hsdir is not None else '/var/lib/tor/testhey%d_hs' % port
...
def parse_and_run(system_arguments):
    ...
    parser_ls.add_argument("-p", "--localport", help='filtering based on local port the HS binds to')
    parser_ls.add_argument("-f", "--format", help='line format, default is %%(onion)s:%%(onionport)d\t%%(address)s\t%%(hsdir)s',
		    dest="fmt", default='%(onion)s:%(onionport)d\t%(address)s\t%(hsdir)s' )
